...
> = {
  ...
  cutoff: {
    ...
    keywords: [
      "MCA cutoff 2025",
      "NIMCET cutoff trends",
      "MCA entrance cutoff history",
      "MAH MCA CET cutoff",
    ],
  },
  salary: {
    ...
    keywords: [
      "MCA salary after graduation",
      "NIT MCA placement",
      "MCA salary by college",
      "MCA career salary 2025",
    ],
  },
  eligibility: {
...
/*
  FIX: JSON-LD structured data added.
  This gives Google rich context about each tool — eligible for sitelinks,
  breadcrumbs, and richer SERP snippets — without any runtime JS cost.
  Uses WebApplication schema (appropriate for interactive tools).
*/
function ToolStructuredData({ slug }: { slug: string }) {
  const meta = TOOL_META[slug];
  if (!meta) return null;
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: "Tools",
          item: "https://crackora.com/tools",
        },
        {
          "@type": "ListItem",
          position: 3,
          name: meta.title,
          item: `https://crackora.com/tools/${slug}`,
        },
      ],
    },
  };
...
export default async function ToolPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <ToolsClient slug={slug as ToolKey} />
    </>
  );
}